...
ubuntu-rebuild:
    ...
    @cd ubuntu && sudo env PATH="$$PATH" nix run 'github:numtide/system-manager' -- switch --flake .#\{{`hostname`}}
...
ubuntu-rebuild-trace:
    ...
    @cd ubuntu && sudo env PATH="$$PATH" nix run 'github:numtide/system-manager' -- switch --flake .#\{{`hostname`}} --show-trace
