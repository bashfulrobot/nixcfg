...
in {
  ...
  config = lib.mkIf cfg.enable {
    # Use official NixOS zoom-us module instead of custom implementation
    programs.zoom-us.enable = true;
  };

  # LEGACY: Previous custom implementation - commented out for rollback if needed
  # Replaced with official NixOS module above
  /*
  { user-settings, pkgs, config, lib, inputs, ... }:
  let
    cfg = config.apps.zoom-us;
  ...
  in {
    ...
    config = lib.mkIf cfg.enable {
      ...
      home-manager.users."${user-settings.user.username}" = {
        ...
        #   [IdXwZwfWMO]
        #   LX8xvOAWJTzobeTVcdsSALgecT\TjwU%23=1709829946
