...
drun)
  ...
  pkill -x rofi 2>/dev/null || true; rofi -show drun -theme-str "$r_override" -theme "$rofi_theme"
  ;;
window)
  rofi_theme="${XDG_CONFIG_HOME:-$HOME/.config}/rofi/launchers/type-2/style-2.rasi"
...
file)
  ...
  pkill -x rofi 2>/dev/null || true; rofi -show filebrowser -theme-str "$r_override" -theme "$rofi_theme"
  ;;
emoji)
  ...
  pkill -x rofi 2>/dev/null || true; rofi -modi emoji -show emoji -theme "${rofi_theme}" -theme-str "$r_override"
  ;;
games)
  ...
  pkill -x rofi 2>/dev/null || true; rofi -show games -modi games -theme "${rofi_theme}" -theme-str "$r_override"
  ;;
help | --help)
